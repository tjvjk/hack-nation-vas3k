...
**Powered by ElevenLabs (The AI Audio and Voice Agents Platform)**
...
| | |
|---|---|
| **Opaque pricing** | Quotes for the same 45-mile move range from $1,158 to $6,506 — a 5.6x spread for identical work. The only sanity check available to a consumer is gathering multiple quotes, and there is no efficient way to do it today. |
| **Unreliable estimates** | Sight-unseen estimates — which is what most phone quotes are — are 40% more likely to end in a final bill above the original quote (FMCSA). Lowball-then-upcharge is a business model, not an accident. |
| **No way to shop around** | The market is 16,851 small companies reachable one phone call at a time. The BBB logs 13,000+ complaints against movers every year, largely over unexpected price hikes — yet almost nobody has the hours to call 5–8 operators and compare properly. |
...
Whatever the vertical, the system always has the same three beats:

1. **Intake** — build a complete, structured specification of the job — through a voice interview, through documents the user already has (photos, existing quotes, bills, inventory lists), or both.
2. **The calls** — your agent phones the market, describes the job identically every time, and extracts itemised quotes.
3. **The close** — it negotiates, leveraging one bid against another, questioning fees, flagging suspicious outliers — and reports back with a ranked, evidence-backed comparison: call recordings, transcripts, and a recommended deal explained in plain language.

Price gathering, reporting, and negotiation are all mandatory; the vertical is yours.

Conversation design is as much part of the challenge as the technology. How your agent handles a gruff dispatcher, a "we don't give prices over the phone", or a hard sell determines whether it comes back with a usable quote or a useless range.
...
Your agent phones the market. How the other end answers is up to you — three setups are all valid:

1. **Real calls** — point the agent at a problem you are actually facing this month and let it phone real businesses via Twilio/SIP
2. **Human-in-the-loop** — answer the calls yourself, playing distinct counterparts: the tough negotiator, the lowballer with hidden fees, the hard-sell upseller
3. **Simulated market** — build counterparty agents and run agent-to-agent

Whatever the setup, the caller must describe the job consistently, handle friction (interruptions, evasive answers, "someone will call you back"), and extract an itemised, comparable quote from each call. Use batch calling or parallel sessions where you can — and show where the call list would come from in the real world (e.g. Google Places or Yelp business data).
...
| Resource | Use |
|---|---|
| ElevenLabs Agents Platform | Build and orchestrate the conversational agents — system prompts, tools, knowledge bases, agent transfer, and human handoff. (elevenlabs.io/docs) |
...
| Resource | Use |
|---|---|
| Google Places / Yelp Fusion / OSM | Find the counterparties: business phone numbers, categories, opening hours, ratings, and review volume by location — your call list, built programmatically instead of by hand. |
...
|---|---|
| Show a real negotiation. Demonstrate a price moving during a live call — against a real business, a role-playing human, or a counter-agent — because of leverage your agent gathered, not because the script said so. | Stage a screenplay. Two agents reading pre-written dialogue at each other is a demo of text-to-speech, not of negotiation. |
| Pick a provable pain. Choose a vertical where you can show the spread with real numbers — then make quotes comparable: same job, same services, itemised fees. | Pick a vertical because it sounds cool, collect prices that aren't comparable, and hide the differences in a polished dashboard. |
| Close the loop. Intake → calls → negotiation → ranked recommendation with transcript evidence. A connected flow beats a polished fragment. | Over-engineer the agent stack and under-engineer the conversations. This challenge is won in call design, not model architecture. |
| Be honest about the hard parts. AI disclosure, honesty constraints, and failure modes (hang-ups, refusals to quote) handled gracefully build trust. | Let the agent bluff. Invented inventory or a fake competing bid may win $200 in the demo — and lose the user, and the market, forever. |
...
- The loop is closed. A single vertical runs end to end: intake → calls → negotiation → ranked recommendation with transcript evidence.
